function visualizeAMSNew(U, Asys, mode, aProduced, ad, abc, pdfFile, doAppend)
    ...
    % NEW: export target + append
    if nargin < 7 || isempty(pdfFile),   pdfFile = 'AMS.pdf'; end
    if nargin < 8 || isempty(doAppend),  doAppend = false; end
    ...
    % --- basic figure setup ---
    if doExport
        figure('Color',[1 1 1], 'Visible','off');  % NEW: headless
    else
        figure('Color',[1 1 1]);
    end
    hold on;
    ...
    if doExport
        exportgraphics(gcf, pdfFile, ...
            'ContentType','vector', 'Padding',5, 'Append', doAppend); 
    end
end
