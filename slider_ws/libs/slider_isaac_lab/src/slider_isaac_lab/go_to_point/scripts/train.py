...
from slider_isaac_lab.utils.sb3_mlflow_wrapper import loggers
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: dict):
    ...
    log_dir = urlparse(mlflow.get_artifact_uri("artifacts")).path
    ...
    policy_cls = CustomActorCriticPolicy  # <— class, not string
    agent_cfg["policy_kwargs"] = dict(
        # log_std_init=-1.5,
        features_extractor_class=DictObsExtractor,   # also a class, not string
        features_extractor_kwargs=dict(features_dim=64),
    )

    agent = PPO(policy_cls, env, verbose=1, **agent_cfg)
    ...
    policy = agent.policy
    policy.save(os.path.join(log_dir, "policy"))
    torch.save(policy.state_dict(), os.path.join(log_dir, "policy.pth"))
    
    print("Saving to:")
    print(os.path.join(log_dir, "policy"))
...
if __name__ == "__main__":
    ...
    mlflow.set_experiment("slider_go_to_point")
    # mlflow.set_experiment("slider_go_to_point_safe")
